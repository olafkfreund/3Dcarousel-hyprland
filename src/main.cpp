...
struct WorkspaceItem {
  ...
  WorkspaceItem(SP<CWorkspace> ws)
      : workspace(ws), position(0, 0), angle(0.0f), scale(1.0f), alpha(1.0f),
        isSelected(false) {}
};
...
public:
  ...
  void loadConfiguration() {
    ...
    try {
      auto *const PRADIUS =
          HyprlandAPI::getConfigValue(PHANDLE, "plugin:carousel:radius");
      if (PRADIUS && PRADIUS->getDataStaticPtr()) {
        m_config.radius =
            std::any_cast<Hyprlang::INT>(*PRADIUS->getDataStaticPtr());
      }

      auto *const PSPACING =
          HyprlandAPI::getConfigValue(PHANDLE, "plugin:carousel:spacing");
      if (PSPACING && PSPACING->getDataStaticPtr()) {
        m_config.spacing =
            std::any_cast<Hyprlang::FLOAT>(*PSPACING->getDataStaticPtr());
      }

      auto *const PALPHA = HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:carousel:transparency_gradient");
      if (PALPHA && PALPHA->getDataStaticPtr()) {
        m_config.transparencyGradient =
            std::any_cast<Hyprlang::FLOAT>(*PALPHA->getDataStaticPtr());
      }

    } catch (...) {
      HyprlandAPI::addNotification(PHANDLE,
                                   "[Carousel] Using default configuration",
  ...
  void setupHooks() {
    ...
    m_pRenderHook = HyprlandAPI::registerCallbackDynamic(
        PHANDLE, "render",
        [this](void *, SCallbackInfo &, std::any) { this->onRender(); });

    m_pWorkspaceChangeHook = HyprlandAPI::registerCallbackDynamic(
        PHANDLE, "workspace",
        [this](void *, SCallbackInfo &, [[maybe_unused]] std::any data) {
  ...
  void toggleCarousel() {
    ...
    if (m_bActive) {
      ...
      HyprlandAPI::addNotification(
          PHANDLE,
          "Carousel activated: " + std::to_string(m_vWorkspaces.size()) +
              " workspaces",
  ...
  void nextWorkspace() {
    if (!m_bActive || m_vWorkspaces.empty())
      return;
    ...
    HyprlandAPI::addNotification(
        PHANDLE, "Next: " + std::to_string(m_iSelectedIndex + 1),
  ...
  void prevWorkspace() {
    ...
    m_iSelectedIndex =
        (m_iSelectedIndex - 1 + m_vWorkspaces.size()) % m_vWorkspaces.size();
    updateSelection();
    calculatePositions();

    HyprlandAPI::addNotification(
        PHANDLE, "Previous: " + std::to_string(m_iSelectedIndex + 1),
        CHyprColor{0.2, 0.2, 1.0, 1.0}, 500);
  }

  void selectWorkspace() {
    if (!m_bActive || m_vWorkspaces.empty() ||
        m_iSelectedIndex >= (int)m_vWorkspaces.size())
      return;
    ...
    if (selectedItem.workspace) {
      ...
      std::string cmd =
          "workspace " + std::to_string(selectedItem.workspace->m_id);
      HyprlandAPI::invokeHyprctlCommand("dispatch", cmd);

      HyprlandAPI::addNotification(
          PHANDLE,
          "Switched to workspace " +
              std::to_string(selectedItem.workspace->m_id),
          CHyprColor{0.2, 1.0, 0.2, 1.0}, 1000);
    }
...
private:
  void updateWorkspaceList() {
    ...
    if (g_pCompositor->m_lastMonitor) {
      auto currentWS = g_pCompositor->getWorkspaceByID(
  ...
  void calculatePositions() {
    ...
    for (size_t i = 0; i < m_vWorkspaces.size(); ++i) {
      ...
      float workspaceAngle =
          (2.0f * M_PI * i) / m_vWorkspaces.size() - m_fRotationAngle;
      ...
      // Final position
      item.position.x =
          centerX + cosf(workspaceAngle) * m_config.radius * perspective;
      ...
      if (item.isSelected) {
        ...
        item.alpha = fmaxf(0.3f, 1.0f - fabsf(z) / m_config.radius *
                                            m_config.transparencyGradient);
      }
    }
  }
  ...
  void onRender() {
    ...
    if (renderCount <= 3) {
      ...
      HyprlandAPI::addNotification(PHANDLE, msg, CHyprColor{0.8, 0.8, 0.2, 1.0},
                                   1000);
    }
  ...
  void renderWorkspaceItem(const WorkspaceItem &item) {
    ...
    try {
      ...
      CBox box{(double)(item.position.x - width / 2),
               (double)(item.position.y - height / 2), (double)width,
               (double)height};

      // Workspace color (varies by index or ID)
      int colorIndex =
          item.workspace ? item.workspace->m_id : (&item - &m_vWorkspaces[0]);
      float hue = (colorIndex * 60.0f) / 360.0f;
      CHyprColor workspaceColor{0.3f + 0.4f * sinf(hue * 2 * M_PI),
      ...
      if (item.isSelected) {
        ...
        CBox borderBox{box.x - 8, box.y - 8, box.width + 16, box.height + 16};
...
SDispatchResult carousel_test(std::string) {
  ...
  HyprlandAPI::addNotification(PHANDLE, status, CHyprColor{0.2, 0.8, 1.0, 1.0},
                               3000);
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
  ...
  HyprlandAPI::addNotification(
      PHANDLE, "[Carousel] Simplified plugin loaded successfully!",
      CHyprColor{0.2, 1.0, 0.2, 1.0}, 3000);

  return {"Hypr Carousel", "3D workspace carousel (simplified)", "Claude",
          "1.0.0"};
}
